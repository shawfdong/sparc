...
class KmerCountingSpec extends FlatSpec with Matchers  with SharedSparkContext
{
  ...
  "kmer counting" should "work on the test seq files" in {
    val cfg = KmerCounting.parse_command_line("-i test/small -k 31 -p sample.seq -o tmp/kmercounting_seq_test.txt --n_iteration 1".split(" ")).get
  ...
  "kmer counting" should "work on the test seq files with canonical_kmer" in {
    val cfg = KmerCounting.parse_command_line("-i test/small -k 31 -C -p sample.seq -o tmp/kmercounting_seq_test_C.txt --n_iteration 1".split(" ")).get
    print(s"called with arguments\n${cfg.valueTreeString}")
    KmerCounting.run(cfg, sc)
  }
  ...
  "kmer counting" should "work on the test base64 files" in {
    val cfg = KmerCounting.parse_command_line("-i test/small -k 31 -p sample.base64 --format base64 -o tmp/kmercounting_base64_test.txt --n_iteration 3".split(" ")).get
    print(s"called with arguments\n${cfg.valueTreeString}")
    KmerCounting.run(cfg, sc)
  }
  ...
  "kmer counting" should "work on fake seq file" in {
    val cfg = KmerCounting.parse_command_line("-i test/small -k 31 -p fake_sample.seq -o tmp/kmercounting_seq_fake.txt --n_iteration 1".split(" ")).get
    print(s"called with arguments\n${cfg.valueTreeString}")
    KmerCounting.run(cfg, sc)
  }
