...
object GraphLPA extends App with LazyLogging {
  ...
  def cc(edgeTuples: RDD[(Long, Long)], config: Config, sqlContext: SQLContext) = {
    cc_graphframes(edgeTuples, sqlContext, config.max_iteration)
  }


  def cc_graphframes(edgeTuples: RDD[(Long, Long)], sqlContext: SQLContext, max_iteration: Int) = {
    ...
    val cc = graph.labelPropagation.maxIter(max_iteration).run()
    val clusters = cc.select("id", "label").rdd.map(x => (x(0).asInstanceOf[Long], x(1).asInstanceOf[Long]))
    clusters
  }
