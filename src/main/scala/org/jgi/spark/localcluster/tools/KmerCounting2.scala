...
object KmerCounting2 extends App with LazyLogging {

  case class Config(input: String = "", output: String = "", n_iteration: Int = 1, pattern: String = "",
                    k: Int = -1, format: String = "seq", sleep: Int = 0,
                    n_partition: Int = 0)
  ...
  def parse_command_line(args: Array[String]): Option[Config] = {
    val parser = new scopt.OptionParser[Config]("KmerCounting") {
      ...
      opt[Int]("n_iteration").action((x, c) =>
        ...
        validate(x =>
          if (x == 1 || x == 4 || x == 16 || x == 64 || x == 256) success
          else failure("n should be in [1,4,16,64,256]"))
  ...
  private def process_iteration(i: Int, readsRDD: RDD[String], config: Config, sc: SparkContext) = {
    val kmer_group_bit = get_kmer_group_bit(i, config.n_iteration)

    val kmer_gen_fun = (seq: String) =>

      Kmer2.generate_kmer(seq = seq, k = config.k, group_bits = kmer_group_bit)

    val smallKmersRDD = process_iteration_spark(i, readsRDD, config, kmer_gen_fun)
    ...
    println(s"filter out ${kmer_count} kmers (count>1) from total ${raw_count} kmers")
  ...
  def get_kmer_group_bit(i: Int, n: Int): String = {
    if (n == 1)
      ""

    else if (n == 4)
      Kmer2.GROUP4.apply(i)
    else if (n == 16)
      Kmer2.GROUP16.apply(i)
    else if (n == 64)
      Kmer2.GROUP64.apply(i)
    else if (n == 256)
      Kmer2.GROUP256.apply(i)
    else
      throw new Exception("unknown for " + n)
  }
  ...
  override def main(args: Array[String]) {
    ...
    options match {
      case Some(_) =>
        ...
        val conf = new SparkConf().setAppName("Kmer Count " + config.input)
