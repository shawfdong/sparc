...
public final class GuavaBloomFilter<T> implements Predicate<T>, Serializable {
    ...
    interface Strategy extends java.io.Serializable {
         ...
         */
        <T> boolean put(T object, Funnel<? super T> funnel, int numHashFunctions, GuavaBloomFilterStrategies.GuavaBitArray bits);
        ...
        <T> boolean mightContain(
                T object, Funnel<? super T> funnel, int numHashFunctions, GuavaBloomFilterStrategies.GuavaBitArray bits);

        /**
         * Identifier used to encode this strategy, when marshalled as part of a GuavaBloomFilter.
         * Only values in the [-128, 127] range are valid for the compact serial form.
         * Non-negative values are reserved for enums defined in GuavaBloomFilterStrategies;
     ...
     */
    final GuavaBloomFilterStrategies.GuavaBitArray bits;
     ...
     */
    GuavaBloomFilter(
            GuavaBloomFilterStrategies.GuavaBitArray bits, int numHashFunctions, Funnel<? super T> funnel, Strategy strategy) {
    ...
    /**
     * Creates a new {@code GuavaBloomFilter} that's a copy of this instance. The new instance is equal to
     * this instance but shares no mutable state.
     *
     * @since 12.0
     */
    @CheckReturnValue
    public GuavaBloomFilter<T> copy() {
        return new GuavaBloomFilter<T>(bits.copy(), numHashFunctions, funnel, strategy);
    }
    ...
    /**
     * Puts an element into this {@code GuavaBloomFilter}. Ensures that subsequent invocations of
     ...
     * expected) have been put in the {@code GuavaBloomFilter}, degenerating it.
     *
     ...
     * @since 15.0
     */
    public void putAll(GuavaBloomFilter<T> that) {
        checkNotNull(that);
        checkArgument(this != that, "Cannot combine a GuavaBloomFilter with itself.");
        checkArgument(
                this.numHashFunctions == that.numHashFunctions,
                "GuavaBloomFilters must have the same number of hash functions (%s != %s)",
                this.numHashFunctions,
                that.numHashFunctions);
        checkArgument(
                this.bitSize() == that.bitSize(),
                "GuavaBloomFilters must have the same size underlying bit arrays (%s != %s)",
                this.bitSize(),
                that.bitSize());
        checkArgument(
                this.strategy.equals(that.strategy),
                "GuavaBloomFilters must have equal strategies (%s != %s)",
                this.strategy,
                that.strategy);
        checkArgument(
                this.funnel.equals(that.funnel),
                "GuavaBloomFilters must have equal funnels (%s != %s)",
                this.funnel,
                that.funnel);
    ...
    public boolean equals(@Nullable Object object) {
        ...
        if (object instanceof GuavaBloomFilter) {
            GuavaBloomFilter<?> that = (GuavaBloomFilter<?>) object;
    ...
    /**
     * Creates a {@link GuavaBloomFilter GuavaBloomFilter<T>} with the expected number of
     * insertions and expected false positive probability.
     * <p>
     * <p>Note that overflowing a {@code GuavaBloomFilter} with significantly more elements
    ...
    public static <T> GuavaBloomFilter<T> create(
        ...
        return create(funnel, expectedInsertions, fpp, GuavaBloomFilterStrategies.MURMUR128_MITZ_64);
    }
     ...
     */
        ...
        try {
            return new GuavaBloomFilter<T>(new GuavaBloomFilterStrategies.GuavaBitArray(numBits), numHashFunctions, funnel, strategy);
     ...
     */
    @CheckReturnValue
    public static <T> GuavaBloomFilter<T> create(Funnel<? super T> funnel, int expectedInsertions) {
     ...
     */
    @CheckReturnValue
    public static <T> GuavaBloomFilter<T> create(Funnel<? super T> funnel, long expectedInsertions) {
   ...
   */
    ...
    private static class SerialForm<T> implements Serializable {
        ...
        SerialForm(GuavaBloomFilter<T> bf) {
        ...
        Object readResolve() {
            return new GuavaBloomFilter<T>(new GuavaBloomFilterStrategies.GuavaBitArray(data), numHashFunctions, funnel, strategy);
        }
    ...
    /**
     * Writes this {@code GuavaBloomFilter} to an output stream, with a custom format (not Java
     * serialization). This has been measured to save at least 400 bytes compared to regular
     * serialization.
     * <p>
     * <p>Use {@linkplain #readFrom(InputStream, Funnel)} to reconstruct the written GuavaBloomFilter.
     */
     ...
     *                     not appear to be a GuavaBloomFilter serialized using the
     *                     {@linkplain #writeTo(OutputStream)} method.
     */
    @CheckReturnValue
    public static <T> GuavaBloomFilter<T> readFrom(InputStream in, Funnel<T> funnel) throws IOException {
        ...
        try {
            ...
            Strategy strategy = GuavaBloomFilterStrategies.values()[strategyOrdinal];
            ...
            IOException ioException = new IOException(
                    "Unable to deserialize GuavaBloomFilter from InputStream."
